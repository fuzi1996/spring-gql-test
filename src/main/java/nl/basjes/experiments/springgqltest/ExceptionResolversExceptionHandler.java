...
class ExceptionResolversExceptionHandler implements DataFetcherExceptionHandler {
    ...
    public CompletableFuture<DataFetcherExceptionHandlerResult> handleException(
        ...
        try {
            return Flux.fromIterable(this.resolvers)
                ...
                .onErrorResume(
                    resolverEx -> Mono.just(handleResolverError(resolverEx, exception, env)))
                .switchIfEmpty(Mono.fromCallable(() -> createInternalError(exception, env)))
                .contextWrite((context) -> {
                    ContextView contextView = getReactorContext(env.getGraphQlContext());
                    return (contextView.isEmpty() ? context : context.putAll(contextView));
                })
                .toFuture();
        } catch (Exception resolverEx) {
            return CompletableFuture
                .completedFuture(handleResolverError(resolverEx, exception, env));
        }
    }
    ...
    private DataFetcherExceptionHandlerResult handleResolverError(
        ...
        if (logger.isWarnEnabled()) {
            logger.warn("Failure while resolving " + originalException.getMessage(),
                resolverException);
        }
    ...
    private DataFetcherExceptionHandlerResult createInternalError(Throwable ex,
        DataFetchingEnvironment environment) {
        ExecutionId executionId = environment.getExecutionId();
        if (logger.isErrorEnabled()) {
            logger.error(
                "Unresolved " + ex.getClass().getSimpleName() + " for executionId " + executionId,
                ex);
        }
